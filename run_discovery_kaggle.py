...
def run_discovery(raw_universe: list[str], tracker: ProgressTracker | None = None):
    ...
    if tracker:
        tracker.set_phase(f"Downloading 2-year data for {len(raw_universe)} stocks")
    df_raw = yf.download(
        raw_universe,
    ...
    if tracker:
        tracker.advance(1, phase=f"Downloaded data for {len(df_raw.columns)} valid stocks", force=True)
    ...
    if tracker:
        tracker.advance(
            1,
            phase=f"Filtered universe to {len(optimized_universe)} symbols",
            force=True,
        )
    ...
    if tracker:
        tracker.advance(1, phase="Preparing Framework 6 node features", force=True)
    node_features = {}
    for symbol in optimized_universe:
        ...
        if tracker:
            tracker.advance(1, phase="Preparing Framework 6 node features")
    ...
    expected_returns = {}
    if tracker:
        tracker.set_phase("Running Framework 6 GNN shock simulation")
    for symbol in optimized_universe:
        try:
        ...
        if tracker:
            tracker.advance(1, phase="Running Framework 6 GNN shock simulation")

    causal_rankings = pd.Series(expected_returns).sort_values(ascending=False)

    if tracker:
        tracker.advance(1, phase="Applying Framework 8 sentiment multiplier", force=True)
    np.random.seed(int(time.time()))
    multipliers = {
    ...
    if tracker:
        tracker.advance(
            1,
            phase=f"Selected long {top_pick_long} / short {top_pick_short}",
            force=True,
        )
